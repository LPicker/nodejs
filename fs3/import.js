...
var fnImportExample = function(src, filename){
	...
	fs.readFile(src + "/" + filename, {
		...
		// 处理动态传入的内容
		var arr_value = dataReplace.match(/\{value:.*\s*\}/ig);
		value = eval(arr_value[0]);
		var $ = cheerio.load(dataReplace, {decodeEntities: false});
		...
		dataReplace = $.html();
		dataReplace = dataReplace.replace(/\{value:.*\s*\}/ig, "");
